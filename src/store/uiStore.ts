...
}), { name: 'taskstride-preferences', partialize: (state) => ({ activeView: state.activeView, sidebarCollapsed: state.sidebarCollapsed, theme: state.theme, density: state.density, horizon: state.horizon, favoriteLists: state.favoriteLists, locale: state.locale }) }))
