...
describe('TaskStride UI', () => {
  ...
  it('shows live progress while tasks are synchronizing', async () => { renderApp(); expect(await screen.findByRole('status', { name: 'Syncing…' })).toBeInTheDocument(); expect(await screen.findByRole('heading', { name: 'All tasks' })).toBeInTheDocument() })
