...
export async function cacheSnapshot(lists: GoogleTaskList[], tasks: TaskWithList[], lastSync = new Date().toISOString()) { await cacheDb.transaction('rw', cacheDb.taskLists, cacheDb.tasks, cacheDb.meta, async () => { await cacheDb.taskLists.clear(); await cacheDb.taskLists.bulkPut(lists); await cacheDb.tasks.clear(); await cacheDb.tasks.bulkPut(tasks); await cacheDb.meta.put({ key: 'lastSync', value: lastSync }) }) }
