...
import {
  ...
  Circle, Cloud, CloudOff, Command, Download, GripVertical, Inbox, Keyboard, ListTodo,
  Menu, Moon, MoreHorizontal, Plus, RefreshCw, Search, Settings, Sparkles, Trash2, WifiOff, X,
} from 'lucide-react'
...
export default function App() {
  ...
  const createTask = async () => {
    ...
    try { const saved = await repository.createTask(list.id, { title, due: temporary.due }); queryClient.setQueryData<WorkspaceData>(['workspace', authVersion], (current) => current ? ({ ...current, tasks: current.tasks.map((task) => task.id === temporary.id ? { ...saved, taskListId: list.id, taskListTitle: list.title } : task) }) : current); selectTask(saved.id) }
  ...
  useEffect(() => {
    ...
    register({ name: 'list_visible_tasks', title: 'List visible tasks', description: 'Read the tasks currently visible in TaskFlow.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: true }, execute: () => ({ view: viewLabel, tasks: visibleTasks.map(({ id, title, status, due, taskListTitle }) => ({ id, title, status, due: dueToDateKey(due), list: taskListTitle })) }) })
...
function Welcome({ onConnect }: { onConnect: () => void }) { const m = messages[useUiStore((store) => store.locale)]; return <main className="welcome"><div className="welcome-card"><span className="welcome-logo"><Check /></span><h1>TaskFlow</h1><p className="welcome-lead">{m.welcomeLead}</p><p>{m.welcomePrivacy}</p><button className="primary-button" onClick={onConnect}>{m.connectGoogle} <ArrowUpRight /></button><button className="text-button" onClick={() => toast(m.welcomePrivacy)}>{m.howItWorks}</button></div></main> }
