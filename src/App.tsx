...
export default function App() {
  ...
  return <div className={`app ${selectedTask ? 'has-details' : ''}`} data-density={density} data-composer={composerOpen ? 'open' : 'closed'} data-sidebar={useUiStore.getState().sidebarCollapsed ? 'collapsed' : 'open'}>
    ...
    <main className="workspace">
      ...
      {/* One slim row: title, filter and actions. On phones only the filter shows here; the title
          lives in the top bar, and the composer is ordered above this row. */}
      <header className="view-header"><h1>{viewLabel} <span className="view-count">({viewTaskCount})</span></h1><label className="task-filter"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={m.filterTasks} aria-label={m.filterTasks} />{query && <button type="button" onClick={() => setQuery('')} aria-label={m.clearFilter}><X /></button>}</label><div className="header-actions"><button className="icon-button" onClick={refresh} aria-label={m.refreshTasks}><RefreshCw className={workspace.isFetching ? 'spinning' : ''} /></button><button className="icon-button" onClick={() => setCommandOpen(true)} aria-label={m.commandMenu}><MoreHorizontal /></button></div></header>
